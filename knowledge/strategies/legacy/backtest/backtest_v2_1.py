...
# -*- coding: utf-8 -*-
"""
BobQuant 基本面 + 技术面双重筛选回测 v2.1
使用 100 只股票池 + 基本面筛选 + 技术面信号
"""
import sys
import os
import yaml
...
print("=" * 60)
print("🚀 BobQuant 基本面 + 技术面双重筛选回测 v2.1")
print("=" * 60)

# 加载 100 只股票池
pool_file = os.path.join(root_dir, 'config', 'stock_pool_v2_100.yaml')
with open(pool_file, 'r', encoding='utf-8') as f:
    stock_pool_raw = yaml.safe_load(f)

print(f"\n📊 股票池配置:")
print(f"  初始股票数：{len(stock_pool_raw)} 只")

# 去重（有些股票重复出现）
seen = set()
stock_pool = []
for s in stock_pool_raw:
...
for s in stock_pool:
    ...
    if any(x in name for x in ['银行', '保险', '证券']):
        industry = '银行金融'
    elif any(x in name for x in ['酒', '饮料', '食品']):
        industry = '食品饮料'
    elif any(x in name for x in ['科技', '半导体', '电子', '网络']):
        industry = '科技/半导体'
    elif any(x in name for x in ['能源', '锂', '光伏']):
        industry = '新能源'
    elif any(x in name for x in ['医药', '医疗', '生物']):
        industry = '医药医疗'
    elif any(x in name for x in ['消费', '家电', '旅游']):
        industry = '消费家电'
    elif any(x in name for x in ['资源', '石化', '黄金', '铝业', '铜业']):
        industry = '周期资源'
    elif any(x in name for x in ['电力', '神华', '煤业', '铁路']):
        industry = '高股息防御'
    else:
        industry = '其他'
...
config = {
    ...
    'min_fundamental_score': 60,  # 基本面最低评分
    'rsi_buy_max': 35,
    'rsi_sell_min': 70,
    'volume_confirm': True,
    'stop_loss_pct': -0.08,
    'max_position_pct': 0.08,  # 单票最大 8% 仓位
    'max_stocks': 15,  # 最大同时持仓 15 只
}
...
for day_idx, date_str in enumerate(trading_days):
    ...
    for stock in stock_pool:
        ...
        if signal == 'buy' and code not in positions:
            ...
            position_size = min(capital * 0.08 / price, max_position_value / price)
...
# 对比历史回测结果
print("\n📊 回测对比:")
print(f"  股票池：    5 只 → 50 只 → 100 只 (双重筛选)")
print(f"  年化收益：  +3.80% → +2.18% → {annual_return*100:+.2f}%")
print(f"  最大回撤：  0.70% → 5.82% → {max_dd*100:.2f}%")
print(f"  夏普比率：  1.83 → 0.42 → {sharpe:.2f}")
print(f"  胜率：      83.3% → 67.1% → {win_rate*100:.1f}%")
...
output_path = os.path.join(root_dir, 'backtest', 'reports', 'fundamental_technical_backtest_2024.json')
